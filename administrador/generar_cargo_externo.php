...
if($_REQUEST['id']=='1')
{
	for($i=1;$i<=$_REQUEST['nro_cargos'];$i++)
	{
		if($_REQUEST['cargo'.$i.'']!='')
		{
			$cargo_externo->cargo_externo_nuevo($_REQUEST['fecha'],$_REQUEST['cargo'.$i.''],$_REQUEST['courier_destino']);
		}
	}
...
if($_REQUEST['id']==='2')
{
	$cargo_externo->cargo_externo_borrar($_REQUEST['campos']);
}	
...
<table width="638" border="0" align="center">
  ...
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td colspan="4"><?php if(($_REQUEST['id']!=1) && ($_REQUEST['id']!=2)){?><input name="Submit" type="button" class="btn" value="Registrar Cargo" onclick="nuevo()" /><?php } ?>
      &nbsp;
      <label>
        <input name="button2" type="button" class="btn" id="button2" value="Nuevo" onclick="remover_campos()" />
      </label>
      <input name="Submit2" type="button" class="btn" value="Asignar cargos a mensajeros" onclick="asignar()" /></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
...
$rs = $cargo_externo->cargo_externo_listar('0',$_REQUEST['fecha'],$_REQUEST['courier_destino']);
...
<table width="245" border="0" align="center">
  <tr class="fondonegro">
    ...
    <td width="59" align="center"><label>
      <input name="button3" type="button" class="btn" id="button3" value="Eliminar" onclick="validar_checkbox_seleccionados(1)" />
    </label></td>
  </tr>
<?php
$total=0;
...
$total = $total +1;
	...
	<tr class="color_celda">
    <td colspan="2" align="center">Cargos registrados: <?php echo $total;?></td>
    </tr>
...
{
	document.forms.form1.action='generar_cargo_externo.php';
